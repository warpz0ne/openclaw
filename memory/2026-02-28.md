...
Checkpoint (2026-02-28 19:11 UTC)
- Done: replaced Google One Tap/iframe button with custom first-party button UX.
- Done: implemented server-side OAuth Authorization Code flow (`/auth/google/start` + `/auth/google/callback`) with `prompt=select_account` for explicit account chooser.
- Done: removed personalized login widget behavior and white Google iframe strip entirely.
- Done: updated setup docs/env requirements to include `GOOGLE_CLIENT_SECRET` and `GOOGLE_REDIRECT_URI`.
- Next step: set new env vars in `/etc/default/slice`, restart service, verify polished login/logout flow on `dudda.cloud`.

Checkpoint (2026-02-28 19:22 UTC)
- Done: created reusable ops scripts area at `tools/ops/` for repetitive debug/deploy cycles.
- Done: added `tools/ops/slicectl.sh` with one-command status/restart/logs/test/env-edit/env-show flows.
- Done: added `tools/ops/README.md` documenting usage.
- Next step: commit + push current changes and use `slicectl.sh` for future Slice operations.

Checkpoint (2026-02-28 19:49 UTC)
- Done: pushed ops helper script to repo (`4313aa4`) for repeatable Slice service workflows.
- Done: login logo direction validated by user (looks good now).
- In progress: finalize custom OAuth login flow UX and production env wiring.
- Critical next step on return: set `GOOGLE_CLIENT_SECRET` and `GOOGLE_REDIRECT_URI` in `/etc/default/slice`, restart service, and verify no white Google widget/chip behavior.

Checkpoint (2026-02-28 23:31 UTC)
- Done: installed additional high-popularity skills into workspace: `ontology`, `self-improving-agent`, `agent-browser`, `tavily-search`, `find-skills`.
- Safety vetting: static scan found no obvious malicious shell patterns in installed files; ClawHub security metadata still flags `find-skills` as suspicious (use cautiously).
- Next step: user enablement plan for first four skills and optional `TAVILY_API_KEY` setup.
